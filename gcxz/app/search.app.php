...
class SearchApp extends MallbaseApp
{
	...
	function index() {
		...
		if (! isset ( $_GET ['goodsId'] ) && empty ( $param )) {
...
// 			exit ();
		...
		$orderStr = $this->_get_query_order ();
		/* 商品列表 */
		$sgrade_mod = & m ( 'sgrade' );
		...
		if (isset ( $_GET ['goodsId'] )) {
			$goodsId = json_decode ( $_GET ['goodsId'] );
		...
		if (isset ( $_GET ['cate_id'] ) && intval ( $_GET ['cate_id'] ) > 0) {
			$this->assign ( 'cateResult', $this->_get_gcategory_by_id ( $goods_mod, $_GET ['cate_id'] ) );
		} else {
			$cates = array_unique ( getSubByKey ( $goods_mod->get_list ( array (
					'conditions' => $conditions 
			) ), 'cate_id' ) );
			$this->assign ( 'cateResult', $this->_get_gcategory_list ( $goods_mod, $cates ) );
		}
		...
		// 获取仁寿区域
		$this->assign ( 'areaArr', $this->_get_area () );
    ...
    function _get_query_param()
    {
        ...
        if ($res === null)
        {
            ...
            if (isset($_GET['price']))
            {
                ...
                $this->assign('min',$arr[0]);
                $this->assign('max',$arr[1]);
    ...
    function _get_area(){
    	...
    	return $regionModel->db->getAll ( $areaSql );
    }
    ...
    function _get_gcategory_list($model,$cates){
    	//获取父类cate_id
	...
	function _get_gcategory_by_id($model,$cate_id) {
		// 获取父类cate_id
		$cateResult = $model->db->getAll ( 'SELECT * FROM ecm_gcategory WHERE cate_id=' . $cate_id );
		$children = $model->db->getAll ( 'SELECT * FROM ecm_gcategory WHERE parent_id=' . $cate_id );
		$cateResult[0] ['children'] = $children;
		return $cateResult;
	}
}
