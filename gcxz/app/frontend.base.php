...
//     	$store_id = intval($this->visitor->info['store_id']);
        ...
        return $tree->getArrayList($cate_id);
    }
    /**
     * 取得全站的分类
     */
    function _get_site_gcategory($cate_id=0)
    {
        return $this->_get_store_gcategory($cate_id, 0);
    }
...
class MemberbaseApp extends MallbaseApp
{
    ...
    {
        /* 只有登录的用户才可访问 */
        if (!$this->visitor->has_login && !in_array(ACT, array('login', 'register', 'check_user','send_reg_code', 'ajax_goods')))
        {
            if (!IS_AJAX)
            {
                header('Location:index.php?app=member&act=login&ret_url=' . rawurlencode($_SERVER['PHP_SELF'] . '?' . $_SERVER['QUERY_STRING']));
...
//                 ),
                ...
                'my_rights'  => array(
                        'text'  => '退款维权',
                        'url'   => 'index.php?app=buyer_refund',
                        'name'  => 'my_rights',
                        'icon'  => 'ico7',
                ),
//                 'my_coupon'  => array(
...
//             );
            ...
            $menu['im_seller']['submenu']['refund_manage'] = array(
                    ...
                    'url'   => 'index.php?app=seller_refund',
                    'name'  => 'refund_manage',
                    'icon'  => 'ico10',
            );
            $menu['im_seller']['submenu']['return_manage'] = array(
                    'text'  => Lang::get('return_manage'),
                    'url'   => 'index.php?app=seller_returngoods',
                    'name'  => 'return_manage',
                    'icon'  => 'ico10',
            );
