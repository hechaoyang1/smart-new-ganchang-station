...
class Seller_orderApp extends StoreadminbaseApp
{
    ...
    {
        list($order_id, $order_info)    = $this->_get_valid_order_info(array(ORDER_SUBMITTED, ORDER_PENDED));
        if (!$order_id)
        {
            echo Lang::get('no_such_order');
    ...
    function finished()
    {
        //TODO 共享仓的商品才支持货到付款，标识为完成由仓储发起通知
        echo Lang::get('no_such_order');
        return;
    ...
    function _get_orders()
    {
        ...
        foreach ($orders as $key1 => & $order)
        {
            foreach ($order['order_goods'] as $key2 => $goods)
            {
                ...
                $order['source_type'] = $goods['source_type'];
            }
        }
