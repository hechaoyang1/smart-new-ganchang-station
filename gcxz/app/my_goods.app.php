...
class My_goodsApp extends StoreadminbaseApp
{
    ...
    function batch_edit()
    {
        if (!IS_POST)
        {
             ...
             /*取得仁寿地区 */
             $this->assign('regions', $this->_get_region_options());
    ...
    function formData()
    {
        ...
        $_get_goods_info = $this->_get_goods_info(0);
		$this->assign('goods', $_get_goods_info);
    ...
    function add()
    {
        $this->_curitem('my_goods');
        $this->_curmenu('goods_add');
        $this->display('my_goods.preadd.html');
    }
    ...
    function addShare()
    {
        ...
        else
        {
            ...
            if(!empty($specs))
            {
                ...
                {
                    if(empty($value['share_goods_id']))
                    {
                        $this->show_warning('请选择共享仓的商品！');
                        return;
                    }
                }
            }
            ...
            /* 保存数据 */
            if (!$this->_save_post_data($data, 0, 2))
            {
                $this->show_warning($this->get_error());
                return;
            }
...
//         $this->_goods_mod->drop_data($ids);
		//查询需要删除商品的信息
        $goods = $this->_goods_mod->db->getAll('select * from ecm_goods where '.db_create_in($ids,'goods_id'));
        $rows = $this->_goods_mod->drop($ids);
        if ($this->_goods_mod->has_error())
        {
            $this->show_warning($this->_goods_mod->get_error());
...
//                  ),
    ...
    /* 取得地区分类，指定parent_id */
    function _get_region_options($parent_id = 511400)
    {
    	$res = array();
    	$mod =& m('region');
    	$regions =  $mod->get_list($parent_id);
    ...
    function _get_post_data($id = 0)
    {
        $goods = array(
            ...
            'description'      => str_replace(SITE_URL.'/', '', $_POST['description']),
            'cate_id'             => $_POST['cate_id'],
            'cate_name'        => $_POST['cate_name'],
            'brand'                  => $_POST['brand'],
        ...
        switch ($goods['spec_qty'])
        {
            case 0: // 没有规格
                $specs[intval($_POST['spec_id'])] = array(
					...
					'price1' => $this->_filter_price($_POST['price1']),
                    'stock' => intval($_POST['stock']),
                    'sku'      => trim($_POST['sku']),
                    'spec_id'  => trim($_POST['spec_id']),
                    'share_goods_id'  => trim($_POST['share_goods_id']),
                );
                break;
            case 1: // 一个规格
                    ...
                    if ($spec_1)
                    {
                        if (($spec_id = intval($_POST['spec_id'][$key]))) // 已有规格ID的
                        {
                            $specs[$key] = array(
								...
								'price1'  => $this->_filter_price($_POST['price1'][$key]),
                                'stock'  => intval($_POST['stock'][$key]),
                                'sku'       => trim($_POST['sku'][$key]),
                                'share_goods_id'  => trim($_POST['share_goods_id'][$key]),
                            );
                        }
                        else  // 新增的规格
                        {
     ...
     * 保存数据
     */
    function _save_post_data($data, $id = 0, $type = 1)
    {
        import('image.func');
        import('uploader.lib');
        ...
        foreach ($data['specs'] as $key => $spec)
        {
            ...
            {
                $spec['goods_id'] = $goods_id;
                $spec['type'] = $type; //商品类型，1：自营，2：共享仓
                $spec_id = $this->_spec_mod->add($spec);
            }
            if (empty($default_spec))
            {
                $default_spec = array('default_spec' => $spec_id, 'price' => $spec['price'],'price1' => $spec['price1']);
            }
        }
