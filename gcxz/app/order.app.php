...
class OrderApp extends ShoppingbaseApp
{
    ...
    function index()
    {
        ...
        if (!IS_POST)
        {
            ...
            //重新算订单费用
            $orders = $order_type->_handle_order_info($goods_info,array());
            $form['data']['orders'] = ecm_json_encode($orders['orders']);
            if ($form === false)
            {
        ...
        else
        {
            ...
            $order_ids = $order_type->submit_order(array(
...
//             $this->_mailto($seller_address, addslashes($seller_mail['subject']), addslashes($seller_mail['message']));
			...
			/* 到收银台付款 */
			header ( 'Location:index.php?app=cashier&act=goto_pay&order_id=' . json_encode($order_ids) );
        }
    }
     ...
     * 检查商品支付模式
     */
    function  checkGoodsPayment(){
		$goodInfo = $this->_get_goods_info ();
		$items = $goodInfo ['items'];
		$goodsModel = & m ( 'goods' );
		$result = array ();
		foreach ( $items as $rec_id => $good ) {
			$goodId = $good ['goods_id'];
			// 获取商品类型
			$goodType = $goodsModel->get ( array (
					'conditions' => "goods_id = '$goodId'",
					'fields' => 'source_type' 
			) );
			if ($goodType ['source_type'] == 1) {
				$result [] = array (
						$good ['rec_id'],
						$good ['goods_name'] 
				);
