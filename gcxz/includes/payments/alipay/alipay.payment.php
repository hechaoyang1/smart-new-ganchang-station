...
class AlipayPayment extends BasePayment
{
    ...
    {
        //TODO 支付宝相关配置信息，这是个临时方案
        $this->_config = include(ROOT_PATH . '/includes/payments/alipay/payment.cfg.php');
    }
    /**
     *    获取支付表单
     *
...
//             '_input_charset'    => CHARSET,//字符集
//             'notify_url'        => $this->_create_notify_url($order_info['order_id']),//异步通知地址
//             'return_url'        => $this->_create_return_url($order_info['order_id']),//支付完成后跳转地址

//             /* 业务参数 */
//             'subject'           => $this->_get_subject($order_info),//商品信息
//             //订单ID由不属签名验证的一部分，所以有可能被客户自行修改，所以在接收网关通知时要验证指定的订单ID的外部交易号是否与网关传过来的一致
//             'out_trade_no'      => $this->_get_trade_sn($order_info),//系统唯一订单号
...
//             /* 买卖双方信息 */
//             'seller_email'      => $this->_config['alipay_account']//卖方支付宝邮箱
//         );
...
//         return $this->_create_payform('GET', $params);
        ...
        $ids = json_decode($order_id);
        
        $order_model =& m('order');
        $og_model =& m('ordergoods');
        foreach ($ids as $id)
        {
            $share = $og_model->isShareGoods($id);
            if($share)
            {
                $edit_data['status']    =   ORDER_ACCEPTED;
            }
            else 
            {
                $edit_data['status']    =   ORDER_PENDED;
            }
            $order_model->edit($id, $edit_data);
        }
        return true;
    }
