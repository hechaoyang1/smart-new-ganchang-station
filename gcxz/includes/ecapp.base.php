...
define('ORDER_PENDED', 12);                   // 买家已付款，等待卖家确认
define('ORDER_ACCEPTED', 20);                  // 卖家已确认
define('ORDER_REQUEST', 21);                  // 仓库已发送下载订单请求 临时状态
define('ORDER_RESPONSE', 22);                  // 仓库已下载订单完毕 即正在出库
define('ORDER_SHIPPED', 30);                // 卖家已发货
define('ORDER_REFUNDED', 50);              // 申请退款
define('RETREAT_ORDER_PASS', 31);                   // 退货申请审核通过
define('RETREAT_ORDER_ACCEPT_FAILED', 32);                   // 验货失败
define('RETREAT_ORDER_ACCEPT_OK', 33);                   // 验货成功
...
EOT;
    ...
    function _run_cron()
    {
        ...
        register_shutdown_function(create_function('', '
            ...
            if (!is_file(ROOT_PATH . "/data/inc/tasks.inc.php"))
            {
                $default_tasks = array(
                    "cleanup" =>
                        array (
                ...
                file_put_contents(ROOT_PATH . "/data/inc/tasks.inc.php", "<?php\r\n\r\nreturn " . var_export($default_tasks, true) . ";\r\n\r\n", LOCK_EX);
            }
            import("cron.lib");
            $cron = new Crond(array(
                "task_list" => ROOT_PATH . "/data/inc/tasks.inc.php",
...
class BaseVisitor extends Object
{
    ...
    function logout()
    {
        ...
        /*清空cookie 使自动登录失效*/
        setcookie('gcxz_in',null,time()-8000000);
    }
