...
/**
    ...
    function respond_notify($order_id, $notify_result)
    {
        ...
        switch ($notify_result['target']) {
            case ORDER_PENDED :
                ...
                break;
            case ORDER_PENDING :
                return true; // 未付款的订单不做任何操作，直接返回。
        }
        /* 支付成功后自营商品状态改为已支付，共享商品改为卖家已确认 */
        $model_order->db->query ( "UPDATE ecm_order o SET `status` = ( CASE WHEN ( SELECT source_type FROM ecm_order_goods WHERE order_id = o.order_id limit 1 ) = 1 THEN {$data['status']} ELSE " . ORDER_ACCEPTED . " END ),pay_time={$data['pay_time']} WHERE {$where}" );
        return $model_order->db->affected_rows ();
    }
    ...
    function _handle_order_info($goods_info, $post)
    {
		...
		foreach ( $goods_info ['items'] as $good ) 
		{
			...
			// 获取商品类型 可换货等
			$goodType = $goods->get ( array (
					'conditions' => "goods_id = '$goodId'",
					'fields' => 'service_type,source_type' 
			) );
			$good['source_type'] = $goodType ['source_type'];
			// 是否可退货
			$typeKey = $goodType ['service_type'] == 1 ? 'back' : 'noback';
			// 是否为共享商品
			$comkey = $goodType ['source_type'] == 1 ? 'common' : 'private';
			$key = $typeKey . '_' . $comkey;
			if (array_key_exists ( $key, $orderInfo ['orders'] )) {
				array_push ( $orderInfo ['orders'] [$key] ['goods'], $good );
			} else {
				$orderInfo ['orders'] [$key] = array (
						'goods' => array (
								$good 
						) 
				);
...
//             'shipping_fee'  =>  $shipping_fee,
	 ...
	 */
	function _get_shipping_fee($goods_info,$shipId) {
		/* 计算配送费用 */
		$shipping_model = & m ( 'shipping' );
		
		$shipping_info = $shipping_model->get ( "shipping_id={$shipId} AND store_id={$goods_info[0]['store_id']} AND enabled=1" );
		if (empty ( $shipping_info )) {
			$this->_error ( 'no_such_shipping' );
		...
		/* 配送费用=首件费用＋超出的件数*续件费用 */
		$shipping_fee = $shipping_info ['first_price'] + ($count-1) * $shipping_info ['step_price'];
		return $shipping_fee;
	}
