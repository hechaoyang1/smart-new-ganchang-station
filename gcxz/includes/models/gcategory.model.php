...
{
    var $top_category = array(1=>'农资用品', 2=>'手机/数码/影音', 3=>'女装/男装/内衣', 4=>'潮流男女鞋/包包', 5=>'电脑/配件/办公', 6=>'摩托/汽车/改装', 7=>'配饰/彩妆/洗护', 8=>'家电/厨卫/五金', 9=>'运动鞋包/服饰/体育', 10=>'家具家纺家装', 11=>'学生用品文具玩具', 12=>'食品饮料酒类', 13=>'母婴儿童用品');
    var $table  = 'gcategory';
    var $prikey = 'cate_id';
    var $_name  = 'gcategory';
    var $_relation  = array(
     ...
     * @return array
     */
    function get_list($parent_id = -1, $shown = false, $fine = false)
    {
        $conditions = "1 = 1";
        $parent_id >= 0 && $conditions .= " AND parent_id = '$parent_id'";
        $shown && $conditions .= " AND if_show = 1";
        $fine && $conditions .= "AND is_fine = 1";

        return $this->find(array(
            'conditions' => $conditions,
            'order' => 'sort_order, cate_id,is_fine',
        ));
    }
    ...
    function get_top_category()
    {
        ...
        foreach ($top_category as $floor=>$cate_name)
        {
            $cate = $this->get(array('conditions' => "cate_name='".$cate_name."' and store_id=0 and if_show=1"));
            $top_category[$floor] = array('cate_id' => $cate['cate_id'], 'cate_name' => $cate_name);
        }
        return $top_category;
    }
}
...
class GcategoryBModel extends GcategoryModel
{
    ...
    function get_children($id, $cached = false)
    {
        ...
        {
            $sql = "SELECT cate_id, cate_name FROM {$this->table} WHERE parent_id = '$id' AND is_show = 1 AND store_id = '{$this->_store_id}'";
            $res = $this->getAll($sql);
        }
