...
$(function()
{
    ...
    $('img[ectype="flex"]').click(function(){
        ...
        if(status == 'open')
        {
            ...
            $.get(selfurl + "&act=ajax_cate", {id: id}, function(data){
                if(data)
                {
                    ...
                    for(var i = 0; i < res.length; i++)
                    {
                        ...
                        {
                            is_fine = "<img src='templates/style/images/positive_disabled.gif' ectype='inline_edit' fieldname='is_fine' fieldid='"+res[i].cate_id+"' fieldvalue='0'/>";
                        }
                        //构造每一个tr组成的字符串，标语添加
			...
			"<td class='align_center'>"+is_fine+"</td>"+
